...
<body>
  <?php
    ...
    if ($_GET) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM student_subject WHERE id = $id";
  ...
  <div class="container pt-4">
    <div class="row">
      ...
      <!-- content -->
      <div class="col-sm-10">
        <h2 class="text-center">แก้ไขข้อมูลคอร์สนักเรียน</h2>
        <form method="post" action="../../src/Admin/Student/process_update_course_student.php" enctype="multipart/form-data" style="margin:40px 20px;">
          <div class="form-group col-md-4" style="padding-left: 0px;">
            <label for="inputAddress">วันที่</label>
            <input id="date" type="date" name="date" class="form-control" value="<?php echo $student_sub["date"]; ?>">
          </div>

          <div class="form-row">
            <div class="col form-group col-md-6">
              <label for="inputEmail4">ชื่อคอร์ส</label>
              <input type="hidden" name="id" value="<?php echo $student_sub["id"]; ?>">
              <select id="subject" class="form-control" name="subject_id">
                <?php
                while ($row = mysqli_fetch_array($subject)) {
                    if ($student_sub['subject_id'] == $row['id']) {
            ...
            <div class="col form-group col-md-6">
              ...
              <select id="teacher" class="form-control" name="teacher_id">
                ...
                while ($row = mysqli_fetch_array($teacher)) {
                    if ($student_sub['teacher_id'] == $row['id']) {
          ...
          <div class="form-group">
            ...
            <input id="date_detail" type="text" name="date_detail" class="form-control" value="<?php echo $student_sub['date_detail'] ?>">
          </div>

          <div class="form-row">
            <div class="form-group col-md-6">
              <label for="inputEmail4">เวลาเรียน</label>
              <input id="time" type="text" name="time" class="form-control" value="<?php echo $student_sub['time'] ?>">
            ...
            <div class="form-group col-md-6">
              ...
              <select id="place" class="form-control" name="place_id">
                ...
                while ($row = mysqli_fetch_array($place)) {
                    if ($student_sub['place_id'] == $row['id']) {
                        echo "<option value=\"{$row['id']}\" selected>{$row['name']}</option>";
                    } else {
                        echo "<option value=\"{$row['id']}\">{$row['name']}</option>";
                    }
          ...
          <?php
                if ($student_sub['payment_status'] == "ยังไม่จ่าย") {
                    echo "<input type=\"radio\" name=\"payment_status\" value=\"ยังไม่จ่าย\" checked> ยังไม่จ่าย";
                    echo "<input type=\"radio\" name=\"payment_status\" value=\"จ่ายแล้ว\"> จ่ายแล้ว";
                } else {
                    echo "<input type=\"radio\" name=\"payment_status\" value=\"ยังไม่จ่าย\"> ยังไม่จ่าย";
                    echo "<input type=\"radio\" name=\"payment_status\" value=\"จ่ายแล้ว\" checked> จ่ายแล้ว";
                }
          ?>
          </div>
            <button id="save" type="submit" class="btn btn-outline-info"><i class="fas fa-save fa-lg icon"></i>บันทึก</button>
            <a class="btn btn-outline-danger" href="datas_course_student.php?id=<?php echo $id ?>" role="button"><i class="fas fa-ban fa-lg icon"></i>ยกเลิก</a>
        </form>
      </div>
    </div>
  </div>
