...
<body>
  ...
  <div class="container pt-4">
    <div class="row">
      ...
      <!-- content -->
      <div class="col-sm-10">
        <h1 class="text-center">แก้ไขข้อมูลนักเรียน</h1>
        <form method="post" action="../../src/Admin/Student/process_update_student.php" enctype="multipart/form-data" style="margin:40px 20px;">
          <div class="form-group col-md-4" style="padding-left: 0px;">
            <label for="inputAddress">วันที่</label>
            <input id="date" type="date" name="date" class="form-control" value="<?php echo $student["date"]; ?>">
          </div>
          <div class="form-row">
            <div class="col form-group col-md-6">
              <label for="inputEmail4">ชื่อ</label>
              <input id="name" type="text" name="name" class="form-control" value="<?php echo $student["name"]; ?>">
              <input type="hidden" name="id" value="<?php echo $student["id"]; ?>">
            </div>
            <div class="col form-group col-md-6">
              <label for="inputEmail4">ชื่อเล่น</label>
              <input id="nickname" type="text" name="nickname" class="form-control" value="<?php echo $student["nickname"]; ?>">
            </div>
          </div>
          <div class="form-row">
            <div class="form-group col-md-6">
              <label for="inputEmail4">อายุ</label>
              <input id="age" type="number" name="age" class="form-control" value="<?php echo $student["age"]; ?>">
            </div>
            <div class="form-group col-md-6">
              <label for="inputPassword4">เบอร์โทรศัพท์</label>
              <input id="tel" type="text" name="tel" class="form-control" value="<?php echo $student["tel"]; ?>">
            </div>
          </div>
          <div class="form-group">
            <label for="inputAddress">โรงเรียน</label>
            <input id="school" type="text" name="school" class="form-control" value="<?php echo $student["school"]; ?>">
          </div>
          <div class="form-group">
            <label for="inputAddress">ที่อยู่</label>
            <input id="address" type="text" name="address" class="form-control" value="<?php echo $student["address"]; ?>">
          </div>
          <div class="form-row">
          ...
          <a class="btn btn-outline-danger" href="datas_student.php?id=<?php echo $student["id"]; ?>" role="button"><i class="fas fa-ban fa-lg icon"></i>ยกเลิก</a>
        </form>
      </div>
    </div>
  </div>
...
<script> 
  $(document).ready(function(){
    $('#save').click(function(e){
      ...
      if (date == '' || name == '' || nickname == '' || age == '' || tel == '' || school == '' || address == '') {
