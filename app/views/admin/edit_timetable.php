...
<body>
  <?php
    ...
    if ($_GET) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM subject_timetable WHERE id = $id";

        $result = mysqli_query($conn, $sql);
        $subject_timetable = mysqli_fetch_array($result);
    }
  ...
  <div class="container pt-4">
    <div class="row">
      ...
      <div class="col-sm-10">
        ...
        <form method="post" action="../../src/Admin/Timetable/process_update_timetable.php" enctype="multipart/form-data" style="margin:40px 20px;">
          <div class="form-row">
            <div class="col form-group col-md-6">
              ...
              <select id="subject" class="form-control" name="subject_id">
                ...
                while ($row = mysqli_fetch_array($subject)) {
                    if ($subject_timetable['subject_id'] == $row['id']) {
            ...
            <div class="col form-group col-md-6">
              ...
              <select id="student" class="form-control" name="student_id">
                ...
                while ($row = mysqli_fetch_array($student)) {
                    if ($subject_timetable['student_id'] == $row['id']) {
          ...
          <div class="form-row">
            <div class="form-group col-md-6">
              ...
              <input id="date_start" type="date" name="date_start" class="form-control" value="<?php echo $subject_timetable['date_start'] ?>">              
            </div>
            <div class="form-group col-md-6">
              ...
              <select id="room" class="form-control" name="room_id">
                ...
                while ($row = mysqli_fetch_array($room)) {
                    if ($subject_timetable['room_id'] == $row['id']) {
                        echo "<option value=\"{$row['id']}\" selected>{$row['name']}</option>";
                    } else {
                        echo "<option value=\"{$row['id']}\">{$row['name']}</option>";
                    }
          ...
          <div class="form-row">
            <div class="form-group col-md-6">
              ...
              <input id="time_start" type="time" name="time_start" class="form-control" value="<?php echo $subject_timetable['time_start'] ?>"> 
            </div>
            <div class="form-group col-md-6">
              <div class="form-group">
              <label for="inputEmail4">เวลาสิ้นสุด</label>
              <input id="time_end" type="time" name="time_end" class="form-control" value="<?php echo $subject_timetable['time_end'] ?>">
              </div>
            </div>
          </div>
          <div class="form-row">
            <div class="form-group col-md-2">
              ...
              <input type="color" name="color" value="<?php echo $subject_timetable['color'] ?>" class="form-control"> 
            </div>
          </div>
